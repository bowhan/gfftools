...
public:
    ...
    union {
        ...
        struct {
            bool is_cds
                :1; //"cds" or "start/stop_codon" features
            bool is_exon
                :1; //"exon" and "utr" features
            bool is_transcript
                :1; //if current feature is *RNA or *transcript
            bool is_gene
                :1; //if current feature is *gene
            bool is_gff3
                :1; //if the line appears to be in GFF3 format
            bool can_discard
                :1; //flag unwanted/unrecognized parent features
            bool skipLine
                :1;
        };
    };
    ...
    GffLine(GffLine* l)
        : _parents(NULL), _parents_len(0), dupline(NULL), line(NULL), llen(0), gseqname(NULL), track(NULL), ftype(NULL), info(
        NULL), fstart(0), fend(0), qstart(0), qend(0), qlen(0), score(0), strand(0), flags(0), exontype(0), phase(0), gene_name(
        NULL), gene_id(NULL), parents(NULL), num_parents(0), ID(NULL) { //a copy constructor
    ...
    GffLine()
        : _parents(NULL), _parents_len(0), dupline(NULL), line(NULL), llen(0), gseqname(NULL), track(NULL), ftype(NULL), info(
        NULL), fstart(0), fend(0), qstart(0), qend(0), qlen(0), score(0), strand(0), flags(0), exontype(0), phase(0), gene_name(
        NULL), gene_id(NULL), parents(NULL), num_parents(0), ID(NULL) {
    }
...
public:
    ...
    GffNameList(int init_capacity = 6)
        : GPVec<GffNameInfo>(init_capacity, true), byName(false) {
        idlast = -1;
...
enum GffPrintMode {
    ...
    pgtfExon, pgtfCDS, pgffAny, //print record as read
    pgffExon, pgffCDS, pgffBoth, };


class GffAttrs
    : public GList<GffAttr> {
public:
    GffAttrs()
        : GList<GffAttr>(false, true, false) {}
...
class GffObj
    ...
    void expandExon(int xovl, uint segstart, uint segend, char exontype, double sc, char fr, int qs, int qe);
...
public:
    ...
    //--------------
    GffObj* finalize(GffReader* gfr, bool mergeCloseExons = false, bool keepAttrs = false, bool noExonAttr = true);
    ...
    int exonOverlapIdx(uint s, uint e, int* ovlen = NULL) {
        ...
        if (s > e) Gswap(s, e);
        s--;
        e++; //to also catch adjacent exons
        for (int i = 0; i < exons.Count(); i++) {
    ...
    void printGxf(FILE* fout
                  , GffPrintMode gffp = pgffExon
                  , const char* tlabel = NULL
    ...
    void printGff(FILE* fout, const char* tlabel = NULL, const char* gfparent = NULL, bool cvtChars = false) {
        printGxf(fout, pgffAny, tlabel, gfparent, cvtChars);
    }

    void printTranscriptGff(FILE* fout
                            , char* tlabel = NULL
                            , bool showCDS = false
                            , const char* gfparent = NULL
                            , bool cvtChars = false
    ...
    char* getSpliced(GFaSeqGet* faseq
                     , bool CDSonly = false
                     , int* rlen = NULL
                     , uint* cds_start = NULL
                     , uint* cds_end = NULL
                     , GList<GSeg>* seglst = NULL
                    );
...
class GfList
    ...
    bool mustSort;
public:
    GfList(bool sortbyloc = false)
        : GList<GffObj>(false, false, false) {
    ...
    void finalize(GffReader* gfr, bool mergeCloseExons, bool keepAttrs = false, bool noExonAttr = true);
...
protected:
    ...
    GffObj* promoteFeature(CNonExon* subp, char*& subp_name, GHash<CNonExon>& pex, bool keepAttr, bool noExonAttr);
...
public:
    GPVec<GSeqStat>
        gseqtable; //table with all genomic sequences, but only current GXF gseq ID indices will have non-NULL
    GffNames* names; //just a pointer to the global static Gff names repository
    GfList gflst; //accumulate GffObjs being read
    GffObj*
    newGffRec(GffLine* gffline, bool keepAttr, bool noExonAttr, GffObj* parent = NULL, GffExon* pexon = NULL, GPVec<
        GffObj>* glst = NULL
             );

    //GffObj* replaceGffRec(GffLine* gffline, bool keepAttr, bool noExonAttr, int replaceidx);
    GffObj* updateGffRec(GffObj* prevgfo, GffLine* gffline, bool keepAttr);
    ...
    GffReader(FILE* f = NULL, bool t_only = false, bool sortbyloc = false)
        : discarded_ids(true), phash(true), gseqtable(1, true), gflst(sortbyloc), gseqStats(1, false) {
    ...
    GffReader(const char* fn, bool t_only = false, bool sort = false)
        : discarded_ids(true), phash(true), gseqtable(1, true), gflst(sort), gseqStats(1, false) {
        gff_warns = gff_show_warnings;
        names = NULL;
        fname = Gstrdup(fn);
        transcriptsOnly = t_only;
