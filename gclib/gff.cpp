...
int GffObj::addExon(GffReader* reader, GffLine* gl, bool keepAttr, bool noExonAttr) {
    ...
    if (isTranscript()) {
        ...
        if (exon_ftype_id < 0
            || exons.Count() == 0) //never assigned a subfeature type before (e.g. first exon being added)
            exon_ftype_id = subf_id;
        else {
            if (exon_ftype_id != subf_id) {
                //
                if (exon_ftype_id == ftype_id && exons.Count() == 1 && exons[0]->start == start
                    && exons[0]->end == end) {
    ...
    int eidx = addExon(gl->fstart, gl->fend, gl->score, gl->phase, gl->qstart, gl->qend, gl->is_cds, gl->exontype);
                ...
                if (exons[oi]->exontype > exontype &&
                    ...
                    if (gff_show_warnings && (exons[oi]->start < segstart || exons[oi]->end > segend)) {
                        GMessage("GFF Warning: unusual segment inclusion: %s(%d-%d) within %s(%d-%d) (ID=%s)\n"
                                 , strExonType(exontype)
                                 , segstart
                                 , segend
                                 , strExonType(exons[oi]->exontype)
                                 , exons[oi]->start
                                 , exons[oi]->end
                                 , this->gffID);
                    }
...
*/
            ...
            if (gff_show_warnings)
                GMessage("GFF Warning: overlapping existing exon(%d-%d) while expanding to %d-%d for GFF ID %s\n"
                         , exons[ni]->start
                         , exons[ni]->end
                         , segstart
                         , segend
...
GffObj* GffReader::gfoFind(const char* id, GPVec<GffObj>*& glst, const char* ctg, char strand, uint start, uint end) {
...
GffObj* GffReader::newGffRec(GffLine* gffline
                             , bool keepAttr
                             , bool noExonAttr
                             , GffObj* parent
                             , GffExon* pexon
...
GffObj* GffReader::updateGffRec(GffObj* prevgfo, GffLine* gffline, bool keepAttr) {
...
GffObj*
GffReader::promoteFeature(CNonExon* subp, char*& subp_name, GHash<CNonExon>& pex, bool keepAttr, bool noExonAttr) {
    ...
    while (nextGffLine() != NULL) {
        ...
        if (gffline->ID && gffline->exontype == 0) {
            ...
            if (prevseen) {
                ...
                if (prevseen->createdByExon()) {
                ...
                } else { //duplicate ID -- but this could also be a discontinuous feature according to GFF3 specs
					...
					*/
                    ...
                    prevseen = newGffRec(gffline, keepAttr, noExonAttr, prevseen->parent, NULL, prevgflst);
                    if (gff_warns) {
        ...
        if (gffline->parents == NULL) {//start GFF3-like record with no parent (mRNA, gene)
            ...
            for (int k = 0; k < kparents.Count(); k++) {
                ...
                if (gffline->is_transcript || gffline->exontype == 0) {//possibly a transcript
                    parentgfo = gfoFind(gffline->parents[i]
                                        , newgflst
                                        , gffline->gseqname
                                        , gffline->strand
                                        , gffline->fstart
                    ...
                    //on the same strand
                    parentgfo =
                        gfoFind(gffline->parents[i], newgflst, gffline->gseqname, gffline->strand, gffline->fstart);
                }
                if (parentgfo != NULL) { //parent GffObj parsed earlier
                    ...
                    if (parentgfo->isGene() && gffline->is_transcript
                        ...
                        if (!(gffline->exontype == exgffIntron
                            && (parentgfo->isTranscript() || parentgfo->exons.Count() > 0))) {
...
void GfList::finalize(GffReader* gfr
                      , bool mergeCloseExons
                      , bool keepAttrs
                      , bool noExonAttr
                     ) { //if set, enforce sort by locus
        ...
        if (mergeCloseExons) {
            ...
            for (int i = 0; i < exons.Count() - 1; i++) {
                ...
                while (ni < exons.Count()) {
                    ...
                    if (gfr != NULL && gfr->gff_warns && dist != 0
                        && (exons[ni]->exontype != exgffUTR && exons[i]->exontype != exgffUTR)) {
...
char* GffObj::getUnspliced(GFaSeqGet* faseq, int* rlen, GList<GSeg>* seglst) {
    if (faseq == NULL) {
        GMessage("Warning: getUnspliced(NULL,.. ) called!\n");
        return NULL;
    }
...
char*
GffObj::getSpliced(GFaSeqGet* faseq, bool CDSonly, int* rlen, uint* cds_start, uint* cds_end, GList<GSeg>* seglst) {
    ...
    if (fspan
        < (int) (end - start + 1)) { //special case: stop coordinate was extended past the gseq length, must adjust
...
void GffObj::printSummary(FILE* fout) {
    ...
    fprintf(fout, "%s\t%c\t%d\t%d\t%4.2f\t%4.1f\n", gffID, strand, start, end, gscore, (float) qcov / 10.0);
}
...
void GffObj::printGxfLine(FILE* fout
                          , const char* tlabel
                          , const char* gseqname
                          , bool iscds
                          , uint segstart
    ...
    if (gff3) {
        ...
        fprintf(fout
                , "%s\t%s\t%s\t%d\t%d\t%s\t%c\t%c\ttranscript_id \"%s\";"
                , gseqname
                , tlabel
                , ftype
                , segstart
...
void GffObj::printGxf(FILE* fout, GffPrintMode gffp, const char* tlabel, const char* gfparent, bool cvtChars) {
    //char tmpstr[255];
    char dbuf[1024];
    if (tlabel == NULL) {
