...
template <class OBJ>
GHash<OBJ>::GHash(GFreeProc* freeProc) {
...
template <class OBJ>
const OBJ* GHash<OBJ>::Add(const char* ky, const OBJ* pdata, bool mrk) {
    int p, i, x, h, n;
...
template <class OBJ>
const OBJ* GHash<OBJ>::shkAdd(const char* ky, const OBJ* pdata, bool mrk) {
    int p, i, x, h, n;
    if (!ky) GError("GHash::insert: NULL key argument.\n");
    GASSERT(fCount < fCapacity);
    h = strhash(ky);
...
template <class OBJ>
OBJ* GHash<OBJ>::Replace(const char* ky, const OBJ* pdata, bool mrk) {
    int p, i, x, h, n;
...
// Remove entry
template <class OBJ>
OBJ* GHash<OBJ>::Remove(const char* ky) {
    int p, x, h, n;
...
// Find entry
template <class OBJ>
bool GHash<OBJ>::hasKey(const char* ky) {
    int p, x, h, n;
...
template <class OBJ>
OBJ* GHash<OBJ>::Find(const char* ky, char** keyptr) {
    int p, x, h, n;
    if (!ky) { GError("GHash::find: NULL key argument.\n"); }
    if (0 < fCount) {
        h = strhash(ky);
...
template <class OBJ>
void GHash<OBJ>::startIterate() {// initialize a key iterator; call
    fCurrentEntry = 0;
}

template <class OBJ>
char* GHash<OBJ>::NextKey() {
    int pos = fCurrentEntry;
    while (pos < fCapacity && hash[pos].hash < 0) pos++;
    if (pos == fCapacity) {
        fCurrentEntry = fCapacity;
...
template <class OBJ>
OBJ* GHash<OBJ>::NextData(char*& nextkey) {
    int pos = fCurrentEntry;
    while (pos < fCapacity && hash[pos].hash < 0) pos++;
    if (pos == fCapacity) {
        fCurrentEntry = fCapacity;
...
template <class OBJ>
int GHash<OBJ>::First() const {
    int pos = 0;
    while (pos < fCapacity) {
        if (0 <= hash[pos].hash) break;
        pos++;
    }
...
template <class OBJ>
int GHash<OBJ>::Next(int pos) const {
...
template <class OBJ>
int GHash<OBJ>::Prev(int pos) const {
    GASSERT(0 <= pos && pos < fCapacity);
