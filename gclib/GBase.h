...
public:
    GDynArray(int initcap = dyn_array_defcap)
        : byptr(false), fArray(NULL), fCount(0),
          fCapacity(initcap)
    { // constructor
        GMALLOC(fArray, fCapacity * sizeof(OBJ));
    }

    GDynArray(const GDynArray &a)
        : byptr(false), fArray(NULL),
          fCount(a.fCount), fCapacity(a.fCapacity)
    { // copy constructor
        GMALLOC(fArray, sizeof(OBJ) * a.fCapacity);
        memcpy(fArray, a.fArray, sizeof(OBJ) * a.fCapacity);
    }
    GDynArray(OBJ *ptr, uint pcap)
        : byptr(true), fArray(ptr), fCount(0), fCapacity(pcap)
    {
    ...
    GDynArray &operator=(const GDynArray &a)
    { // assignment operator
...
#define GDYNARRAY_ADD(item) \
    ...
    uint Add(OBJ *item)
    { // Add item to the end of array by pointer
    ...
    uint Add(OBJ item)
    { // Add item copy to the end of array
        GDYNARRAY_ADD(item);
        return (fCount - 1);
    }
    ...
    uint Count()
    { return fCount; } // get size of array (elements)
    uint Capacity()
    { return fCapacity; }
...
public:
    ...
    { return lbuf(); }
    int readcount()
    { return lcount; } //number of lines read
    void setFile(FILE *stream)
    { file = stream; }
    int length()
    { return lbuf.Count(); }
    int size()
    { return lbuf.Count(); } //same as size();
    bool isEof()
    { return isEOF; }
    bool eof()
    { return isEOF; }
    ...
    void pushBack()
    { if (lcount > 0) pushed = true; } // "undo" the last getLine request
    // the next getLine() will in fact return the same line
    GLineReader(const char *fname)
        : closeFile(false), lbuf(1024), isEOF(false), file(NULL),
    ...
    GLineReader(FILE *stream = NULL, off_t fpos = 0)
        : closeFile(false), lbuf(1024), isEOF(false), file(stream),
