...
public:
    ...
    GList(GCompareProc* compareProc, //unsorted by default
        GFreeProc* freeProc, bool beUnique = false
         );
...
template <class OBJ>
GArray<OBJ>::GArray(GArray<OBJ>& array)
    :GVec<OBJ>(0) { //copy constructor
...
template <class OBJ>
const GArray<OBJ>& GArray<OBJ>::operator=(GArray<OBJ>& array) {
...
template <class OBJ>
GArray<OBJ>::GArray(GCompareProc* cmpFunc)
    :GVec<OBJ>(0) {
...
template <class OBJ>
GArray<OBJ>::GArray(bool sorted, bool unique)
    :GVec<OBJ>(0) {
    fUnique = unique;
...
template <class OBJ>
GArray<OBJ>::GArray(int init_capacity, bool sorted, bool unique)
    :GVec<OBJ>(init_capacity) {
    fUnique = unique;
    fCompareProc = sorted ? DefaultCompareProc : NULL;
}

template <class OBJ>
void GArray<OBJ>::setSorted(GCompareProc* cmpFunc) {
...
template <class OBJ>
int GArray<OBJ>::IndexOf(OBJ& item) {
    int result = 0;
...
template <class OBJ>
bool GArray<OBJ>::Exists(OBJ& item) {
    int result = 0;
...
template <class OBJ>
void GArray<OBJ>::Add(GArray<OBJ>& list) {
...
template <class OBJ>
bool GArray<OBJ>::Found(OBJ& item, int& idx) {
...
template <class OBJ>
void GArray<OBJ>::Insert(int idx, OBJ* item) {
...
template <class OBJ>
void GArray<OBJ>::Move(int curidx, int newidx) {
...
template <class OBJ>
void GArray<OBJ>::Replace(int idx, OBJ& item) {
    //TEST_INDEX(idx);
...
template <class OBJ>
GList<OBJ>::GList(GList<OBJ>& list)
    :GPVec<OBJ>(list) { //copy constructor
    fUnique = list.fUnique;
    fCompareProc = list.fCompareProc;
}

template <class OBJ>
GList<OBJ>::GList(GList<OBJ>* plist)
    :GPVec<OBJ>(0) { //another copy constructor
...
template <class OBJ>
void GList<OBJ>::Add(GList<OBJ>& list) {
    if (list.Count() == 0) return;
    if (SORTED) {
...
template <class OBJ>
GList<OBJ>::GList(GCompareProc* compareProc, GFreeProc* freeProc, bool beUnique) {
...
template <class OBJ>
GList<OBJ>::GList(GCompareProc* compareProc) {
    fCompareProc = compareProc;
...
template <class OBJ>
GList<OBJ>::GList(bool sorted, bool free_elements, bool beUnique) {
    if (sorted) {
        if (free_elements) {
            fCompareProc = &DefaultCompareProc;
            this->fFreeProc = GPVec<OBJ>::DefaultFreeProc;
...
template <class OBJ>
GList<OBJ>::GList(int init_capacity, bool sorted, bool free_elements, bool beUnique)
    :GPVec<OBJ>(init_capacity, free_elements) {
    if (sorted) {
        fCompareProc = &DefaultCompareProc;
        fUnique = beUnique;
    } else {
...
template <class OBJ>
const GList<OBJ>& GList<OBJ>::operator=(GList& list) {
    if (&list != this) {
        GPVec<OBJ>::Clear();
        fCompareProc = list.fCompareProc;
...
template <class OBJ>
void GList<OBJ>::setSorted(GCompareProc* compareProc) {
    GCompareProc* old_proc = fCompareProc;
    fCompareProc = compareProc;
    if (fCompareProc != old_proc && fCompareProc != NULL)
        Sort(); //new compare method
...
template <class OBJ>
int GList<OBJ>::IndexOf(OBJ* item) {
    int result = 0;
    if (Found(item, result)) return result;
    else return -1;
}
...
template <class OBJ>
bool GList<OBJ>::Exists(OBJ& item) {
    int result = 0;
    if (Found(&item, result)) return true;
    else return false;
}
...
template <class OBJ>
OBJ* GList<OBJ>::AddIfNew(OBJ* item, bool deleteIfFound, int* fidx) {
    int r;
    if (Found(item, r)) {
...
template <class OBJ>
int GList<OBJ>::AddedIfNew(OBJ* item) {
    int r;
    if (Found(item, r)) {
...
template <class OBJ>
bool GList<OBJ>::Found(OBJ* item, int& idx) {
    //search the list by using fCompareProc (if defined)
    //or == operator for a non-sortable list
    //for sorted lists, even when the result is false, the idx is
    //set to the closest matching object!
...
template <class OBJ>
void GList<OBJ>::sortInsert(int idx, OBJ* item) {
...
template <class OBJ>
void GList<OBJ>::Insert(int idx, OBJ* item) {
    //idx can be [0..fCount] so an item can be actually added
...
template <class OBJ>
void GList<OBJ>::Move(int curidx, int newidx) {
    BE_UNSORTED; //cannot do this in a sorted list!
    GPVec<OBJ>::Move(curidx, newidx);
}

template <class OBJ>
void GList<OBJ>::Put(int idx, OBJ* item, bool re_sort) {
